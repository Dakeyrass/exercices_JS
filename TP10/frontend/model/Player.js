...
export default class Player {
  constructor(id, name, skinPath, position) {
    ...
    // --- RENDER positions ---
    this.renderX = position[0]; //server pos
    this.renderY = position[1];
    this.prevX = this.renderX;
    this.prevY = this.renderY;
    this.finalX = position[0];
    this.finalY = position[1];
  ...
  interpolate(alpha) {
    this.finalX = this.prevX + (this.renderX - this.prevX) * alpha;
    this.finalY = this.prevY + (this.renderY - this.prevY) * alpha;
    console.log("-------------------------");
